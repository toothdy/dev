...
const authJWTPackagePath = "github.com/toothdy/cool-admin-go-next/cool-next/auth/jwt"
...
func Render(model *Model, graph *Graph, descriptors *DescriptorSet) ([]byte, error) {
	...
	if hasAuthProviders(graph.Providers()) {
		imports.add(authPackagePath, "auth")
		imports.add(authJWTPackagePath, "authjwt")
	}
...
func writeInfrastructureDeclarations(source *strings.Builder, fragments []DescriptorFragment, hasOutbox, hasHTTP, hasAuth bool) {
	...
	if hasAuth {
		...
		source.WriteString("\t\t\tJWT authjwt.Config `json:\"jwt\"`\n")
	...
	if hasAuth {
		source.WriteString("\tdefaults.Cool.Auth.JWT = authjwt.DefaultConfig()\n")
...
func writeGeneratedFunction(
	...
	if hasAuthProviders(graph.Providers()) {
		source.WriteString("\tjwtService, err := authjwt.New(infrastructure.Cool.Auth.JWT)\n")
