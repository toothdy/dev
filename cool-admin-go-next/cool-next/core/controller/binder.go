...
func BindAdd[E any, ID comparable](
...
) (service.AddInput[E], error) {
	var zero service.AddInput[E]
	...
	if isMany {
		return service.NewAddArray[E, ID](descriptor, items)
	}

	return service.NewAddObject[E, ID](descriptor, items[0])
}
...
func BindDelete[E any, ID comparable](
	...
	request *ghttp.Request,
	descriptor coreentity.Descriptor[E, ID],
) (service.DeleteInput[ID], error) {
	var zero service.DeleteInput[ID]
	if _, err := binder.require(request); err != nil {
		return zero, err
	}
	body, err := binder.readBody(request)
	...
	return service.NewDeleteInput[E](descriptor, ids)
}
...
func BindUpdate[E any, ID comparable](
	...
	request *ghttp.Request,
	descriptor coreentity.Descriptor[E, ID],
) (service.UpdateInput[E, ID], error) {
	var zero service.UpdateInput[E, ID]
	if _, err := binder.require(request); err != nil {
		return zero, err
	}
	body, err := binder.readBody(request)
	...
	items := make([]service.UpdateItem[E, ID], len(rawItems))
	...
	for index, raw := range rawItems {
		...
		items[index], err = service.NewUpdateItem(descriptor, id, mutable)
		if err != nil {
			return zero, err
		}
	}
	if isMany {
		return service.NewUpdateArray(descriptor, items)
	}

	return service.NewUpdateObject(descriptor, items[0])
}
...
func BindCRUDQuery(binder *Binder, request *ghttp.Request, action crud.Action) (service.Query, error) {
	if _, err := binder.require(request); err != nil {
		return service.Query{}, err
	}
	if action != crud.ActionList && action != crud.ActionPage {
		return service.Query{}, exception.Validate("查询动作必须是 list 或 page")
	}
	body, err := binder.readBody(request)
	if err != nil {
		return service.Query{}, err
	...
	if action == crud.ActionList {
		return service.NewListQuery(queryRequest, size)
	}

	return service.NewQuery(queryRequest, page, size)
}
...
func decodeEntityItems[E any, ID comparable](
	...
	removePrimary bool,
	limit int,
) ([]*service.Mutable[E], bool, error) {
	rawItems, isMany, err := decodeRawObjects(body, limit)
	if err != nil {
		return nil, false, err
	}
	items := make([]*service.Mutable[E], len(rawItems))
	for index, raw := range rawItems {
		if removePrimary {
...
func decodeMutable[E any, ID comparable](
	...
	descriptor coreentity.Descriptor[E, ID],
) (*service.Mutable[E], error) {
	fields := make([]service.FieldValue, 0, len(raw))
	for name, encoded := range raw {
		...
		if bytes.Equal(bytes.TrimSpace(encoded), []byte("null")) {
			fields = append(fields, service.Null(name))
			continue
		}
		target := field.GoType()
	...
	return service.NewMutable[E, ID](descriptor, fields)
}
