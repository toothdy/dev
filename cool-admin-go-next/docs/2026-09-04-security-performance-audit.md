...
本次审计原确认 4 项应优先处理的安全问题，其中验证码泄漏与公开 HTML 存储型 XSS 已在当前工作区修复：

1. 验证码答案包含在客户端 SVG 响应中，验证码可被直接绕过（已修复）；
2. 公开 HTML 接口原样输出数据库内容，存在存储型 XSS 风险（已修复）；
...
### SEC-02：公开原始 HTML 存在存储型 XSS 风险（已修复）

严重级别：高，若参数只能由完全可信管理员维护且页面不在认证主域使用，可下调

修复状态：当前工作区已为公开路由复用 `allowKeys` 键名白名单（默认为空时全拒），标题经 `html.EscapeString` 转义、正文经 bluemonday `UGCPolicy` 白名单清洗后输出，`<script>`、事件属性和 `javascript:` 链接被剥离，富文本结构保留；管理员预览接口共用同一清洗输出。新增防泄漏回归测试，并更新既有断言以匹配文本转义语义。部署层加固（严格 CSP、独立来源）仍建议按上线检查执行。以下证据描述修复前的根因。
...
2. 已完成：限制公开 HTML 键名并完成 HTML 清洗（allowKeys + bluemonday 白名单）；
