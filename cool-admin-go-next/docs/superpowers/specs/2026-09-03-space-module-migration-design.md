...
- 加载顺序：`0`；
- 无模块中间件和全局中间件；
- WPS 默认 `appId`：`SX20230111NDUAGQ`。

Space 配置增加 `deletePhysicalFile`，默认值为 `false`。
...
1. 检查提交的 `url` 是否属于当前 Base 本地上传公开地址；
2. 属于受管本地文件时，从 URL 解析并覆盖 `key`，保持 Midway 本地模式由服务端推导 `key` 的行为；
3. 外部 URL 或未来对象存储 URL 不执行本地路径推导，保留请求中的 `key`；
4. 将规范化后的输入交给基础 Service 新增。

Base 只暴露受管文件定位所需的只读方法，不执行 Space 删除，也不增加删除路由。

### 6.2 删除文件信息

文件信息删除流程：

1. 在当前 CRUD 事务中按请求 ID 查询待删记录的 `key` 与 `url`；
2. 调用基础 Service 删除数据库记录并进入现有删除归档流程；
3. `deletePhysicalFile=false` 时结束，不访问文件系统；
4. `deletePhysicalFile=true` 时，仅解析由 Base 确认为本地受管文件的路径；
5. 使用受根目录约束的文件 API 删除普通文件，不跟随符号链接，不接受绝对路径、目录穿越、非法日期或嵌套文件名；
6. 外部文件和非受管路径跳过物理删除；
7. 文件已经不存在视为成功；其他文件系统错误返回 Core 异常，由 CRUD Dispatcher 回滚数据库事务。
...
5. 路径是否受管必须由服务端配置和规范化结果决定，不能信任客户端提交的 `key` 或 `url`；
...
3. 本地上传 URL 的 `key` 推导与外部 URL 保留；
4. 物理删除默认关闭和显式开启；
5. 单个、批量及分类级联删除；
6. 文件不存在的幂等行为；
7. 外部 URL、绝对路径、目录穿越、非法日期、嵌套文件名、目录和符号链接保护；
